...
const ShoppingCart = ({ className, addHandler }) => {
  ...
  return (
    <div
      className={`${className} max-sm:w-90 relative border-dashed border-gray-200 h-full grid hover:bg-f1-light cursor-pointer transition-all select-none`}
      onClick={addHandler ? addHandler : dummyNumberHandler}
    >
      <button className="place-self-center">
        <img className="max-sm:w-[24px]" src="/images/shopping-cart.svg" alt="" />
      </button>
